...
MARKER="## Learn toolkit"

install_skill() {
  local dest_dir="$1" label="$2"
  mkdir -p "$dest_dir/learn"
  cp "$ROOT/stubs/SKILL.md" "$dest_dir/learn/SKILL.md"
  echo "✓ $label → $dest_dir/learn/SKILL.md"
}
...
echo "Installing learn pointers from $ROOT"
validate_frontmatter
...
if ! command -v node >/dev/null 2>&1; then
  echo
  echo "! node was not found on PATH — learn.mjs needs it (see test-evidence ADR 0001)."
fi

echo
echo "After installing, initialize a vault once:"
echo "  node $ROOT/learn.mjs init --vault \"<path-to-obsidian-vault>\""
echo "Learner data lives in <vault>/.learn/ and syncs with the vault's git remote."