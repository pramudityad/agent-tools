...
install_skill() {
  ...
  mkdir -p "$dest_dir/test-evidence"
  cp "$ROOT/stubs/SKILL.md" "$dest_dir/test-evidence/SKILL.md"
  echo "✓ $label → $dest_dir/test-evidence/SKILL.md"
}
...
echo "Installing test-evidence pointers from $ROOT"
validate_frontmatter
...
echo "Any agent can also be pointed at the toolkit directly:"
echo "  follow $ROOT/TRIAGE.md"
echo "  follow $ROOT/CAPTURE.md"

if ! command -v node >/dev/null 2>&1; then
  echo
  echo "! node was not found on PATH — evidence.mjs needs it (see docs/adr/0001)."
fi
