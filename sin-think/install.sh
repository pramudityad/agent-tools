...
MARKER="## sin-think toolkit"

install_skill() {
  local dest_dir="$1" label="$2"
  mkdir -p "$dest_dir/sin-think"
  cp "$ROOT/stubs/SKILL.md" "$dest_dir/sin-think/SKILL.md"
  echo "✓ $label → $dest_dir/sin-think/SKILL.md"
}
...
try:
    ...
    print(f'frontmatter parse error: {err}'); sys.exit(1)
if not isinstance(data, dict) or 'name' not in data or 'description' not in data:
    print('frontmatter must define name and description'); sys.exit(1)
PY
}
...
[ -d "$HOME/.config/pi" ] && install_skill "$HOME/.config/pi/skills" "pi"
...
if ! command -v node >/dev/null 2>&1; then
  echo
  echo "! node was not found on PATH — sin-think.mjs needs it (see test-evidence ADR 0001)."
fi
