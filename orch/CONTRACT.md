...
| `bad_input` | malformed recipe, unknown command, unknown run id, or an unresolved `{n}` placeholder | read the message; nothing was run |
...
| :--- | :--- | :--- |
| `run <recipe> [args...]` | `audit.jsonl` | runs every step in declared order; stops at the first failure |
